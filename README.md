...
🚀 使用流程
開啟網站：訪問 hkautotools.github.io。

匯入檔案：將多份 PDF 檔案拖放至上傳區。

座標校準：在畫布上框選欲辨識的欄位（如單據編號位置）。

設定規則：輸入 Regex 規則（例如：RO#:[A-Z]{3}\d{8}）與檔案後綴。

同步全案：點擊「套用至全部檔案」，系統將自動處理清單中的所有 PDF。

批量下載：確認預覽名稱正確後，點擊「開始批量改名下載」。
...
🚀 How to Use
Launch: Visit hkautotools.github.io.

Upload: Drag and drop PDF files into the workspace.

Select Zone: Drag your mouse over the PDF area containing the target text.

Configure: Set your Regex patterns and filename suffixes.

Sync: Click "Apply to All" to update OCR results for the entire list.

Export: Review the new filenames and click "Start Batch Processing".
